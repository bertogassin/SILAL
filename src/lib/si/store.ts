...
export const useSi = create<SiStore>()(
  persist(
    (set, get) => ({
      ...
      restorePublic: (raw) => {
        try {
          if (raw.length > 1_500_000) return { ok: false };
          const data = JSON.parse(raw) as Partial<SiPublicState> & { product?: string; vault?: unknown; mnemonic?: unknown };
          if (data.product && data.product !== "si" && data.product !== "SILAL") return { ok: false };
          if (data.vault || data.mnemonic) return { ok: false };
    ...
    }),
    {
      ...
      merge: (persisted, current) => {
        ...
        let ledger = p.ledger ?? current.ledger;
        try {
