...
import { todayDeeds, sayingOfDay } from "@/lib/si/today";
import { LIVE_ROOMS, SOCKETS } from "@/lib/si/blueprint";
...
export function HomeScreen() {
  ...
  const deeds = useMemo(
    ...
    [people, edges, familyAirdropPaid, lastBackupAt, events, rsvps, familyDocs, cases, studyNotes, pots, circle, household],
  );
  const saying = sayingOfDay();
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4">
      ...
      <section className="si-today p-4">
        <p className="si-kicker">{t("today.kicker")}</p>
        <h2 className="mt-1 font-display text-xl">{t("today.title")}</h2>
        <p className="mt-2 font-serif text-sm italic text-pretty text-muted-foreground">{t(saying)}</p>
        {deeds.length > 0 ? (
          <ul className="mt-3 grid gap-2">
            {deeds.map((d) => (
              <li key={d.id}>
                <Link
                  to={d.href}
                  className="flex min-h-11 items-center justify-between gap-3 rounded-xl border border-border px-3 text-sm"
                >
                  <span>{t(d.key)}</span>
                  <span className="shrink-0 text-gold">{t("today.do")}</span>
                </Link>
              </li>
            ))}
          </ul>
        ...
        <form
          ...
          onSubmit={(e) => {
            e.preventDefault();
            if (who.trim()) {
              addPerson({ name: who.trim(), gender: "unknown", living: true, consentToPublish: false });
              setWho("");
        ...
        >
          <Input value={who} onChange={(e) => setWho(e.target.value)} placeholder={t("craft.person")} />
          <Input value={note} onChange={(e) => setNote(e.target.value)} placeholder={t("craft.note")} />
      ...
      <Card>
        <CardContent className="p-4">
          <p className="si-kicker">{t("home.balance")}</p>
      ...
      {soon && (
        <Link to="/events" className="si-door p-4">
          <p className="si-kicker">{t("home.soon")}</p>
      ...
      {(openCases > 0 || unsigned > 0 || pendingRef) && (
        <section>
          ...
          <div className="grid gap-2">
            ...
            {unsigned > 0 && (
              <Link to="/house" className="rounded-2xl border border-border bg-card p-4">
      ...
      <section className="grid grid-cols-2 gap-3">
        ...
        <Link to="/referrals" className="si-door p-4">
          <div className="si-kicker">{t("home.ref")}</div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Link to="/chronicle" className="si-door p-4">
          <div className="font-display">{t("nav.chronicle")}</div>
          <p className="mt-1 text-sm text-muted-foreground">{t("chr.kicker")}</p>
        </Link>
      </div>

      <section>
        <p className="si-kicker">{t("map.live")}</p>
        ...
        <p className="si-kicker mt-4">{t("map.socket")}</p>
        <div className="mt-2 flex flex-wrap gap-2">
          {SOCKETS.map((s) => (
            <span key={s.id} className="rounded-full border border-dashed border-wool/40 px-3 py-1.5 text-xs text-wool">
              {t(s.key)}
            </span>
          ))}
        </div>
